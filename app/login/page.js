...
function LoginInner() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // Onboarding gate: do they have a resume on file?
      const profileRes = await fetch('/api/profile', { cache: 'no-store' })
      const profile = profileRes.ok ? await profileRes.json() : null
      const hasResume = !!profile?.profile?.has_resume

      router.push(hasResume ? next : `/profile?onboarding=1&next=${encodeURIComponent(next)}`)
      router.refresh()
    } catch (e) {
      ...
      <div className="container mx-auto max-w-md">
        ...
        <form onSubmit={onSubmit} className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-xl shadow-indigo-500/5 space-y-5">
          ...
          <p className="text-center text-xs text-slate-400 pt-2">
            New here?{' '}
            <Link href={`/signup${next ? `?next=${encodeURIComponent(next)}` : ''}`} className="font-black text-indigo-400 hover:text-indigo-800">
              Create an account
            </Link>
          </p>
        </form>
