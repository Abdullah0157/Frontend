        ...
        <div className="container mx-auto max-w-md">
          <div className="bg-slate-900 border border-emerald-800 rounded-[2.5rem] p-10 shadow-xl text-center">
            ...
            <p className="text-slate-300 text-sm leading-relaxed">
              We sent a confirmation link to <strong>{form.email}</strong>. Click it to verify your address, then come back and log in.
            </p>
            <Link href="/login" className="inline-block mt-6 text-xs font-black uppercase tracking-widest text-indigo-400 hover:text-indigo-800">
              ← Go to login
            </Link>
          </div>
        </div>
      ...
      <div className="container mx-auto max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tight text-white">
            Join <span className="text-indigo-400">JobStream</span>
          </h1>
          <p className="text-slate-400 mt-3 text-sm">Upload your resume once, interview for any role.</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-xl shadow-indigo-500/5 space-y-5">
          <label className="block">
            <span className="text-xs font-black uppercase tracking-[0.3em] text-slate-300">Full Name</span>
            <input
              type="text"
              required
              value={form.fullName}
          ...
          <label className="block">
            <span className="text-xs font-black uppercase tracking-[0.3em] text-slate-300">Email</span>
            <input
              type="email"
              required
              autoComplete="email"
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-5 py-4 rounded-2xl border border-slate-700 hover:bg-slate-900 transition text-sm font-bold text-slate-200"
          >
          ...
          <p className="text-center text-xs text-slate-400 pt-2">
            ...
            <Link href={`/login${next ? `?next=${encodeURIComponent(next)}` : ''}`} className="font-black text-indigo-400 hover:text-indigo-800">
              Log in
            </Link>
          </p>
        </form>
