      ...
      <div className="min-h-screen pt-32 pb-24 px-4">
        <div className="container mx-auto max-w-md">
          <div className="bg-white border border-emerald-200 rounded-[2.5rem] p-10 shadow-xl text-center">
            ...
            <Link href="/login" className="inline-block mt-6 text-xs font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-800">
              ← Go to login
            </Link>
          </div>
        </div>
      ...
      <div className="container mx-auto max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tight text-slate-900">
            Join <span className="text-indigo-600">JobStream</span>
          </h1>
          <p className="text-slate-500 mt-3 text-sm">Upload your resume once, interview for any role.</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-xl shadow-indigo-500/5 space-y-5">
          <label className="block">
            <span className="text-xs font-black uppercase tracking-[0.3em] text-slate-700">Full Name</span>
            <input
              type="text"
              required
              value={form.fullName}
          ...
          <p className="text-center text-xs text-slate-400 pt-2">
            ...
            <Link href={`/login${next ? `?next=${encodeURIComponent(next)}` : ''}`} className="font-black text-indigo-600 hover:text-indigo-800">
              Log in
            </Link>
          </p>
        </form>
