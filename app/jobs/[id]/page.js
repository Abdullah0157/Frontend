    ...
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="bg-white border-b border-slate-200 py-6">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/jobs" className="text-slate-500 hover:text-indigo-600 text-sm flex items-center mb-4 transition-colors">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center space-x-6">
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200">
                <img src={job.logo} alt={`${job.company} logo`} className="w-full h-full object-cover" />
              </div>
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-slate-900 mb-1">{job.title}</h1>
          ...
          <aside className="w-full lg:w-80 flex-shrink-0">
            <div className="space-y-6 sticky top-24">
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <h3 className="font-bold text-slate-900 mb-4 uppercase text-xs tracking-wider">Job Overview</h3>
                <div className="space-y-4">
                  <div className="flex justify-between items-center py-2 border-b border-gray-50">
                    <span className="text-slate-500 text-sm">Date Posted</span>
                    <span className="text-slate-900 text-sm font-medium">{job.posted_at}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-gray-50">
                    <span className="text-slate-500 text-sm">Openings</span>
                    <span className="text-slate-900 text-sm font-medium">{job.openings_count}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-gray-50">
                    <span className="text-slate-500 text-sm">Pay</span>
                    <span className="text-indigo-600 text-sm font-bold">{job.salary}</span>
                  </div>
                  <div className="flex justify-between items-center py-2">
                    <span className="text-slate-500 text-sm">Referral Bonus</span>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <h3 className="font-bold text-slate-900 mb-4 uppercase text-xs tracking-wider">Required Skills</h3>
                <div className="flex flex-wrap gap-2">
                  {job.tags.map((tag, index) => (
                    <span key={index} className="bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-xs font-medium border border-slate-200">
                      {tag}
                    </span>
                  ))}
                </div>
