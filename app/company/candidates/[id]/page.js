...
function decisionBadge(decision) {
  if (decision === 'hire') return { label: 'Hired', cls: 'bg-emerald-950/40 text-emerald-400 border-emerald-800' }
  if (decision === 'reject') return { label: 'Rejected', cls: 'bg-red-950/40 text-red-400 border-red-800' }
  ...
  if (error) {
    return (
      <div className="min-h-screen pt-32 px-4 text-center">
        ...
        <Link href="/company/candidates" className="text-indigo-400 underline">← Back to candidates</Link>
      </div>
    )
  }
  const { candidate, interviews } = data
  if (!candidate) return <div className="min-h-screen pt-32 text-center text-slate-500">Not found</div>

  return (
    <div className="min-h-screen pt-32 pb-24 px-4">
      <div className="container mx-auto max-w-4xl">
        <Link href="/company/candidates" className="text-xs font-black uppercase tracking-widest text-indigo-400 hover:text-indigo-300">
          ← All candidates
        </Link>
        ...
        {/* Resume text */}
        <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-6 md:p-8 mb-6">
          <h2 className="text-xs font-black uppercase tracking-[0.3em] text-slate-400 mb-4">Resume</h2>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-6 md:p-8">
          ...
          {interviews.length === 0 ? (
            <p className="text-slate-500 text-sm">This candidate hasn't interviewed for any of your jobs yet.</p>
          ) : (
              ...
              {interviews.map((iv) => {
                ...
                return (
                  <Link
                    ...
                    className="block bg-slate-800 border border-slate-700 hover:border-indigo-700 rounded-2xl p-4 transition-all"
                  >
                    <div className="flex items-center justify-between gap-4 mb-2">
                      <p className="font-black text-white text-sm">{iv.job_title}</p>
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      {iv.job_fit_score != null && (
                        <p className="text-indigo-400 font-black">{iv.job_fit_score}% fit</p>
                      )}
                    </div>
                  </Link>
                )
