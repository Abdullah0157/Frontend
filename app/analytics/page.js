    ...
    <div className="min-h-screen bg-slate-50 pt-32 pb-20 px-4 md:px-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          ...
          <div className="flex gap-4">
            <div className="px-6 py-3 bg-white border border-slate-200 rounded-2xl shadow-sm flex items-center gap-3">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-slate-700">Live Tracking Active</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {[
            ...
            <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all group">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4">{item.label}</p>
              ...
              {stats.topJobs.map((job, i) => (
                <div key={i} className="flex items-center justify-between p-6 rounded-3xl bg-slate-50 hover:bg-slate-100 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Applies</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mt-12 text-center">
           <Link href="/" className="text-xs font-black text-slate-500 uppercase tracking-[0.4em] hover:text-indigo-600 transition-colors">← Back to Platform</Link>
        </div>
      </div>
    </div>
  )
