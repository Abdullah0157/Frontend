...
const StatItem = ({ initialValue, suffix, label, colorClass }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] font-sans leading-tight group-hover:text-indigo-400 transition-colors">
        {label}
      </p>
    </div>
  )
...
const ArticleCard = ({ category, readTime, title, excerpt, link, gradientClass, badgeText }) => (
  <div className="max-w-6xl mx-auto bg-slate-900 rounded-[4rem] overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-xl transition-all duration-500 group mb-20">
    <div className={`w-full md:w-1/2 relative min-h-[400px] p-12 flex items-center justify-center overflow-hidden ${gradientClass}`}>
      ...
      <div className="relative z-10 bg-slate-900/10 backdrop-blur-md border border-white/20 px-6 py-2 rounded-full text-[10px] font-black text-white uppercase tracking-[0.4em] transform translate-y-32 group-hover:translate-y-0 transition-all duration-700">
    ...
    <div className="w-full md:w-1/2 p-12 md:p-20 flex flex-col justify-center">
      <div className="flex items-center gap-3 mb-8">
        <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] font-sans">{category}</span>
      ...
      <p className="text-slate-400 text-lg leading-relaxed mb-12 font-sans font-medium">
        {excerpt}
      </p>
      
      <Link href={link} className="text-sm font-black text-white uppercase tracking-widest border-b-2 border-slate-900 pb-1 hover:text-indigo-400 hover:border-indigo-600 transition-all flex items-center w-fit font-sans">
        READ ARTICLE 
...
export default function BlogPage() {
  ...
  const stats = [
    { value: '50', suffix: 'K+', label: 'REMOTE PROFESSIONALS', color: 'text-white' },
    { value: '500', suffix: '+', label: 'COMPANIES HIRING', color: 'text-white' },
    { value: '1', suffix: 'K+', label: 'JOBS POSTED', color: 'text-white' },
    { value: '150', suffix: '+', label: 'COUNTRIES SUPPORTED', color: 'text-white' }
  ]

  const categories = ['ALL', 'CAREER TIPS', 'SKILLS & DEVELOPMENT']

  return (
    <div className="bg-slate-900 min-h-screen pt-48 pb-24 font-serif">
      {/* SECTION 1: HERO */}
      <section className="container mx-auto px-4 mb-20">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-5xl md:text-7xl font-black text-white mb-16 leading-tight tracking-tighter">
            Remote work, <br />
            simplified for you.
          </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
            ...
            <div>
              ...
              <Link href="#articles" className="text-sm font-black text-white uppercase tracking-widest border-b-2 border-slate-900 pb-1 hover:text-indigo-400 hover:border-indigo-600 transition-all flex items-center w-fit font-sans">
      ...
      {/* SECTION 2: STATS */}
      <section className="border-t border-b border-slate-800 py-16 mb-20">
            ...
            <button
              ...
              className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.4em] transition-all border font-sans ${
                activeCategory === cat 
                  ...
                  : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-900 hover:text-white'
              }`}
            >
              {cat}
            </button>
