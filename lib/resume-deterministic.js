...
export function extractContact(text) {
  ...
  // Resumes often set the name in ALL CAPS as a design choice ("ALI USMAN").
  // That's the same name, but it reads badly in a profile UI, so normalise the
  // casing while leaving names that are already mixed-case ("McDonald") alone.
  const rawName = (lines.find(looksLikeName) || '').trim()
  const full_name = rawName && rawName === rawName.toUpperCase()
    ? rawName.toLowerCase().replace(/\b[a-z]/g, c => c.toUpperCase())
    : rawName

  // "City, Country" in the header block. Headers also carry things like
  // "Available for Remote, Contract Work", which matches the same shape — so
  // reject anything containing availability/employment vocabulary.
  const NOT_A_PLACE = /\b(available|remote|onsite|on-site|hybrid|contract|freelance|full|part|time|work|hire|hiring|open|relocat|immediate|notice|willing|seeking)\b/i
  let city = '', country = ''
  for (const l of lines.slice(0, 8)) {
    for (const part of l.split(/[|•·]/)) {
      const t = part.trim()
      if (NOT_A_PLACE.test(t)) continue
      const m = t.match(/^([A-Z][A-Za-z .'-]{2,25}),\s*([A-Z][A-Za-z .'-]{2,25})$/)
