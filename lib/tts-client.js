...
export async function playServerTTS(text, { voice, onStart, onEnd, onError } = {}) {
  ...
  // Prefer STREAMING playback (MediaSource): audio starts on the first chunk that
  // arrives instead of after the whole MP3 downloads + decodes — the single
  // biggest cut to time-to-first-word. The <audio> autoplay block is pre-unlocked
  // in primeTTS() (user gesture), so play() from this async path is allowed.
  if (isMSESupported()) {
    await _playStreaming(text, { voice, onStart, onEnd, onError })
    return
  }

  // Fallback for browsers without MSE-for-mpeg (e.g. Safari): buffered Web Audio
  // via the gesture-resumed AudioContext, then a plain blob as last resort.
