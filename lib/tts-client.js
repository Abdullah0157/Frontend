...
export async function playServerTTS(text, { voice, onStart, onEnd, onError } = {}) {
  ...
  // Prefer Web Audio API — controlled by AudioContext which is resumed in the user
  // gesture (primeTTS in PermissionGate), so it never hits the HTMLAudioElement
  // autoplay block that fires when play() is called from an async effect.
