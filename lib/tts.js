...
export function primeTTS() {
  ...
  try {
    if (window.speechSynthesis) {
      // Empty utterance to mark the gesture-time speak() that unlocks the session.
      // Using a real audible character (not space, not volume=0) because some Chrome
      // versions don't count silent utterances as legitimate unlocks.
      const u = new SpeechSynthesisUtterance('.')
      u.volume = 0.01
      u.rate = 10  // make it nearly instantaneous
