...
// then a smaller/higher-throughput model — instead of failing the whole request.
//
// 2026-08: Groq RETIRED the llama-3.x chat models. The previous defaults
// (llama-3.3-70b-versatile / llama-3.1-8b-instant) now 404, which silently killed
// the entire fallback: whenever Gemini was rate-limited or down, every AI call
// failed with no backup. Verified against the live model list and swapped to the
// current gpt-oss pair. NOTE: these are reasoning models — they spend tokens
// thinking before answering, so callers must allow a realistic max_tokens
// (a tiny budget returns empty content).
...
function toOpenAIBody(geminiBody, model, stream = false) {
  ...
  // Groq bills a request as input + max_tokens against its per-minute budget
  // (8k TPM on the free tier), so a generous max_tokens makes even a SHORT
  // prompt fail with "Request too large". Gemini has no such coupling, so we
  // clamp only here and leave the caller's Gemini budget untouched.
  const requested = generationConfig.maxOutputTokens ?? 1000
  // 4000 keeps input+output under 8k for every resume we've measured (largest
  // was ~3.6k input tokens), while still being ample for a full parse.
  const max_tokens = Math.min(requested, Number(process.env.GROQ_MAX_TOKENS) || 4000)

  return { model, messages, temperature: generationConfig.temperature ?? 0.7, max_tokens, stream }
}

async function callGroq(geminiBody, opts = {}) {
  ...
  // Latency-critical callers (a live interview question) pass a small cap so a
  // "try again in 25s" hint never freezes the turn — we fail fast instead and
  // let the client degrade gracefully. Patient callers (report generation) omit
  // it and keep the original wait-it-out behavior. Default = unbounded.
  const maxWaitMs = opts.maxRateLimitWaitMs ?? Infinity

  let lastErr = { status: 500, data: { error: { message: 'Groq unavailable' } } }
  // Up to 3 passes. Each pass tries every (key × model). A per-minute rate limit
  // (TPM) is transient — Groq tells us "try again in Ns" — so if a whole pass is
  // rate-limited we wait that long and retry, instead of failing the request.
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    for (const key of keys) {
      for (const model of GROQ_MODELS) {
        try {
          ...
          if (res.status === 429) {
            sawRateLimit = true
            // Respect Groq's "try again in Ns" hint when present.
            const m = /try again in ([\d.]+)s/i.exec(data?.error?.message || '')
            if (m) retryAfterMs = Math.max(retryAfterMs, Math.ceil(parseFloat(m[1]) * 1000) + 500)
          } else if (res.status < 500) {
            // Non-retriable (auth, bad request) — surface immediately.
    ...
    if (sawRateLimit && attempt < 2) {
      const wait = retryAfterMs || 6000
      // If the required wait exceeds what this caller allows, retrying after a
      // shorter sleep would just 429 again — bail now and fail fast.
