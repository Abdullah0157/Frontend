...
// seniority. Deliberately below the population median at junior levels and
// rising with scope. LABELED PRIOR — configurable per org, calibrated later.
export const DEFAULT_BAR = { ic3: -0.8, senior: -0.4, staff: 0.1, exec: 0.4 }
...
export function decideHire({ theta, se, coverage, criticalFailures = [], seniority = 'senior', barOverride, cost = DEFAULT_COST }) {
  ...
  if (!Number.isFinite(theta) || coverage < 0.6) {
    return {
      ...
      reason: `Only ${Math.round((coverage || 0) * 100)}% of decision-relevant competencies were adequately evidenced — insufficient to decide.`,
    }
  }
  ...
  let band
  if (p >= 0.85) band = 'Strong Hire'
  else if (p >= 0.70) band = 'Hire'
  else if (p >= 0.58) band = 'Hire with Reservations'
  else if (p >= 0.45) band = 'Borderline'
  else if (p >= 0.25) band = 'No Hire'
  ...
  return {
    ...
    reason: `P(clears bar θ≥${bar}) = ${Math.round(p * 100)}% vs hire threshold ${Math.round(c * 100)}%, at ${Math.round(coverage * 100)}% coverage.`,
  }
}
